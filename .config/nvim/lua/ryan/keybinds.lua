...
nmap('<Leader>so', '<Cmd>ReloadConfig<CR>', { silent = true, noremap = true })
...
nmap('<leader>f',':Telescope diagnostics<cr>')
...
vim.api.nvim_set_keymap('n', '<leader>a', '', {
  ...
  callback = function()
    ...
    if replacement ~= nil and replacement ~= '' then
      ...
      -- THE PATTERN IS THE PROBLEM, works otherwise
      --local pattern = '\\<' .. word .. '\\>'
      --local pattern = '\\v(\\s|[.,;:!?]|^)\\Q' .. word .. '\\E(\\s|[.,;:!?]|$)'
      --local pattern = '\\v\\<\\Q' .. word .. '\\E\\>'
      --local pattern = '%f[%w]' .. vim.fn.escape(word, '\\') .. '%f[%W]'
      --local pattern = '%f[%w]'.. word ..'%f[%W]'
      local pattern = '%f[%w]' .. word .. '%f[%W]'

      -- Construct the substitution command
      --local cmd = string.format("%%s/%s/%s/g", word, replacement)
      print("%%s  word: " .. word .. "  |Replacement: " .. replacement)
